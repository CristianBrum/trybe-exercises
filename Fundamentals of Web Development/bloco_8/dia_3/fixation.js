...
const fullNamess = personss.map((person) => `${person.firstName} ${person.lastName}`);

console.log(fullNamess); // [ 'Maria Ferreira', 'João Silva', 'Antonio Cabral' ]

///////////////////////////////////////////////////////////

const numbersss = [1, 2, 3, 4, -5];

const negativeNumbers = numbersss.map((number) => ((number > 0) ? number * (-1) : number));
...
for (let index = 0; index < numberss.length; index += 1) {
  if (numberss[index] > 0) {
    negativeNumberss.push(numberss[index] * -1);
  } else {
    negativeNumberss.push(numberss[index]);
  }
}
